...
def read_files(folder_name='data'):
    ...
    """
    ratings = pd.read_csv(folder_name + '/ratings.csv.zip')
    books = pd.read_csv(folder_name + '/books.csv.zip')
...
def get_recomendation_df(ids, distances, name_mapper, author_mapper):
    ...
    * distance - значение метрики расстояния до книги
    ...
    recomendation_df = pd.DataFrame({'book_name': names, 
                                     'book_author': authors, 
                                     'distance': distances}
                                     )
...
if option:
    ...
    count_recomendation = st.number_input(
                                          label="Specify the number of recommendations you need / Укажите количество рекомендаций, которое вам нужно", 
                                          value=10
                                          )    
    ...
    st.markdown('Most simmilar books are / Самыми похожими книгами являются: ')
    ...
    fig = px.bar(
                 ...
                 title='Cosine distance to the nearest books / Косинусное расстояние до ближайших книг'
                 )
